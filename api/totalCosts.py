def subjects_order_cost(subjects_order):
    """
    Calculates percentage of soft constraints - order of subjects (S, E, P).
    :param subjects_order: dictionary where key = (name of the subject, index of the group), value = [int, int, int]
    where ints represent start times (row in matrix) for types of classes S, E respectively. If start time is -1
...
def empty_space_groups_cost(groups_empty_space):
    ...
    for group_index, times in groups_empty_space.items():
        ...
        for i in range(1, len(times) - 1):
            ...
            b = times[i]
            diff = b - a
            # classes are in the same day if their time div 10 is the same
            if a // 10 == b // 10 and diff > 1:
                empty_per_day[a // 10] += diff - 1
                cost += diff - 1
...
# free_hour function: this helps to find the time(hour) of the day when no class is held.
...
def free_hour(matrix):
    ...
    for i in range(len(matrix)):
        ...
        if exists:
            return '{}: {}'.format(days[i // 10], hours[i % 10])
...
def hard_constraints_cost(matrix, data):
    """
    Calculates total cost of hard constraints: every classroom is at most one class at a time, every class is in one
    of its possible classrooms, every teacher holds at most one class at a time and every group attends at most one
    class at a time.
