...
from utilities import load_data, show_timetable, show_timetable2, set_up, show_statistics, write_solution_to_file
from totalCosts import check_hard_constraints, hard_constraints_cost, empty_space_groups_cost, empty_space_teachers_cost, \
    free_hour
import copy
import math
...
def valid_teacher_group_row(matrix, data, index_class, row):
    ...
    for j in range(len(matrix[row])):
        #print(f"{matrix[row][j]} is matrix[row][j]")
...
def main():
    ...
    data = load_data('uploads/test_docs/' + file, teachers_empty_space, groups_empty_space, subjects_order)
