import json
import random
from totalCosts import check_hard_constraints, subjects_order_cost, empty_space_groups_cost, empty_space_teachers_cost, \
    free_hour
...
def load_data(file_path, teachers_empty_space, groups_empty_space, subjects_order):
    ...
    for cl in data['Classes']:
        new_group = cl['Programs']
        new_teacher = cl['Lecturer']
...
#---- create a list for teachers too
        # add teacher
...
def set_up(num_of_columns):
    ...
    :return: matrix, free
    """
    w, h = num_of_columns, 50                                          # 5 (workdays) * 10 (work hours) = 50
...
def show_timetable2(matrix):
    """
    Prints timetable matrix.
    """
    days = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"]
    hours = [8, 9, 10, 11, 12, 13, 15, 16, 17, 18]
    f = open('solution_files/show_timetable.txt', 'w')
    # f.write('{')
    # print heading for classrooms
    for i in range(len(matrix[0])):
        if i == 0:
    ...
    alldays = {}
    container_list = []
    matrix.append([None, None, None, None, None, None])
    days.append("")
    for i in range(len(matrix)):
        if (i > 0 ) and (i % 10 == 0) and h_cnt == 10:
            alldays[days[d_cnt]]=container_list
            container_list = []
            h_cnt = 0
            d_cnt += 1
            print()
        ...
        hour = hours[h_cnt]
        print('{:10s} {:2d} ->  '.format(day, hour), end='')
        container_list.append(matrix[i])
        for j in range(len(matrix[i])):
            print('{:6s}'.format(str(matrix[i][j])), end='')
        #     toBeContained_list.append()
        # container_list.append(toBeContained_list)
        print()
        h_cnt += 1
    # print(filled)
    # print(alldays)
    f.write(f'{alldays}')
    f.close()
    jsonTable = open('../src/show_timetable.json', 'a')
    # jsonTable.write("{")
    json.dump(alldays, jsonTable, sort_keys=False)
    jsonTable.write('}')
    jsonTable.close()
    # f.write('}')
...
def write_solution_to_file(matrix, data, filled, filepath, groups_empty_space, teachers_empty_space, subjects_order):
    ...
    """
    f = open('solution_files/solution_' + filepath, 'w')
    jsonTable = open('../src/show_timetable.json', 'w')
    jsonTable.write('{ "classes":')
    ...
    for class_index, times in filled.items():
        ...
        f.write('\nClassroom: {:2s}\nTime: {}'.format(room[:room.rfind('-')], days[times[0][0] // 10]))
        for time in times:
            f.write(' {}'.format(hours[time[0] % 10]))
        classIndex_name["_"+str(class_index)] = c.subject
    f.close()
    json.dump(classIndex_name, jsonTable, sort_keys=False)
    jsonTable.write(', "Allocations":')
    jsonTable.close()
